    ...
    <Card className="bg-black/80 border-teal-500/30 backdrop-blur-md">
      ...
      <CardContent className="pointer-events-auto relative z-10">
        <div className="w-full">
          {/* Custom Tab Buttons */}
          <div className="grid grid-cols-5 gap-1 bg-gray-900/50 rounded-lg p-1 mb-4">
            {marketData.map((data) => (
              <button
                key={data.symbol}
          ...
          {/* Tab Content */}
          {marketData.map((data) => {
            if (data.symbol !== selectedSymbol) return null
            
            return (
              <div key={data.symbol} className="mt-4">
                ...
                <Button
                  className="mt-4 w-full bg-teal-500 hover:bg-teal-600 text-black font-bold pointer-events-auto"
                  onClick={() =>
